  ...
  return (
    <Dialog onOpenChange={(open) => !open && setActiveIdx(0)}>
      <DialogTrigger asChild>
        <article className="group cursor-pointer bg-[#F7F6F2] border border-black/10 hover:border-[#1E3FE8] transition-colors">
          <div className="relative aspect-[4/3] overflow-hidden bg-black/5">
            ...
            {hasVariants && (
              <div className="absolute bottom-3 left-3 flex items-center gap-1.5 bg-[#0A0A0F]/85 text-white px-2.5 py-1 text-[10px] uppercase tracking-wider font-semibold">
                <Images size={12} />{variants.length} Designs
              </div>
            )}
          ...
          <div className="p-5">
            ...
            <p className="mt-2 text-sm text-foreground/70 leading-relaxed">{p.short}</p>
            <div className="mt-4 flex flex-wrap gap-1.5">
              {p.tags.map((t) => (
                <span key={t} className="text-[10px] uppercase tracking-wider font-semibold text-foreground/60 border border-black/15 px-2 py-1">{t}</span>
              ))}
            </div>
          </div>
        </article>
      ...
      <DialogContent className="max-w-4xl w-[calc(100%-1.5rem)] p-0 overflow-hidden rounded-none border-black/15 max-h-[92vh] overflow-y-auto [&>button:last-of-type]:hidden">
        ...
        <div className="grid md:grid-cols-2">
          <div className="relative bg-black/5">
            <div className="aspect-square w-full">
              <img src={activeImage} alt={activeVariant ? activeVariant.name : p.name}
                className="w-full h-full object-cover transition-opacity duration-300" key={activeImage} />
            </div>
            {activeVariant && (
              <div className="absolute bottom-3 left-3 bg-white/95 px-3 py-2 shadow-md">
                <div className="text-[10px] uppercase tracking-[0.18em] text-[#1E3FE8] font-bold">Design</div>
                <div className="font-display font-bold text-sm tracking-tight mt-0.5">{activeVariant.name}</div>
              </div>
            )}
          </div>
          ...
          <div className="p-6 md:p-7">
            <DialogHeader className="text-left space-y-1">
              <span className="text-[10px] uppercase tracking-[0.2em] text-[#1E3FE8] font-bold">{p.category}</span>
              <DialogTitle className="font-display font-black text-2xl tracking-tight pr-8">{p.name}</DialogTitle>
            </DialogHeader>
            <p className="mt-3 text-sm text-foreground/75 leading-relaxed">{activeVariant?.desc || p.description}</p>

            <div className="mt-5 border-t border-black/10">
              {p.specs.map((s) => (
                <div key={s.label} className="flex justify-between gap-4 py-2.5 border-b border-black/10">
                  <span className="text-[11px] uppercase tracking-wider text-foreground/60 font-semibold">{s.label}</span>
            ...
            <div className="mt-6 flex gap-2">
              <Button asChild className="bg-[#1E3FE8] hover:bg-[#1733C0] text-white rounded-none font-semibold">
                <a href="#contact">Request Quote</a>
              </Button>
              <Button asChild variant="outline" className="rounded-none border-black/30">
                <a href={`https://wa.me/${COMPANY.phoneRaw}?text=Hi, I'm interested in ${encodeURIComponent(activeVariant ? `${p.name} - ${activeVariant.name}` : p.name)}`}
        ...
        {hasVariants && (
          <div className="border-t border-black/10 bg-[#F7F6F2] p-5 md:p-6">
              ...
              {variants.map((v, i) => (
                <button key={i} type="button" onClick={() => setActiveIdx(i)}
                  className={`relative group text-left bg-white border-2 transition-all ${
                    i === activeIdx ? "border-[#1E3FE8] shadow-md" : "border-transparent hover:border-black/30"
                  }`}>
                  <div className="aspect-square overflow-hidden bg-black/5">
                    <img src={v.image} alt={v.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="px-1.5 py-1.5">
                    <div className="text-[10px] font-semibold uppercase tracking-wider truncate">{v.name}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] uppercase tracking-[0.22em] text-[#1E3FE8] font-bold">Our Products</span>
            </div>
          ...
          <p className="text-foreground/70 lg:text-right max-w-md">
            From driveways to highways, decorative jalis to heavy-duty cable covers — our 11 product categories cover every concrete need.
          </p>
        </div>
          ...
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setActive(c)}
              className={`px-4 py-2 text-xs font-semibold uppercase tracking-wider border transition-colors ${
                active === c ? "bg-[#0A0A0F] text-white border-[#0A0A0F]" : "bg-white text-foreground border-black/15 hover:border-black/40"
              }`}>
              {c}
            </button>
          ))}
